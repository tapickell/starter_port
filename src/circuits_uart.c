...
// Global starter_port reference
static struct starter_port *starter_port = NULL;
static struct starter_port_config current_config;
...
static int parse_option_list(const char *req, int *req_index, struct starter_port_config *config)
{
    int term_type;
    int option_count;
    ...
    for (int i = 0; i < option_count; i++)
    {
        ...
        else if (strcmp(key, "parity") == 0)
        {
            ...
            if (strcmp(parity, "none") == 0)
                ...
                config->parity = starter_port_PARITY_EVEN;
            else if (strcmp(parity, "odd") == 0)
                config->parity = starter_port_PARITY_ODD;
            else if (strcmp(parity, "space") == 0)
                config->parity = starter_port_PARITY_SPACE;
            else if (strcmp(parity, "mark") == 0)
                config->parity = starter_port_PARITY_MARK;
            else if (strcmp(parity, "ignore") == 0)
                config->parity = starter_port_PARITY_IGNORE;
        }
        else if (strcmp(key, "flow_control") == 0)
        {
            ...
            else if (strcmp(flow_control, "hardware") == 0)
                config->flow_control = starter_port_FLOWCONTROL_HARDWARE;
            else if (strcmp(flow_control, "software") == 0)
                config->flow_control = starter_port_FLOWCONTROL_SOFTWARE;
        }
        else
        {
            // unknown term
...
static void handle_open(const char *req, int *req_index)
{
    ...
    struct starter_port_config config = current_config;
    ...
    // If the starter_port was already open, close and open it again
    if (starter_port_is_open(starter_port))
        starter_port_close(starter_port);

    if (starter_port_open(starter_port, name, &config) >= 0)
    {
    ...
    {
        send_error_response(starter_port_last_error());
    }
}
...
static void handle_configure(const char *req, int *req_index)
{
    ...
    if (starter_port_configure(starter_port, &config) >= 0)
    {
        current_config = config;
        send_ok_response();
    }
...
static void handle_configuration(const char *req, int *req_index)
{
    ...
    case starter_port_PARITY_NONE:
        ei_encode_atom(resp, &resp_index, "none");
        break;
    case starter_port_PARITY_EVEN:
        ei_encode_atom(resp, &resp_index, "even");
        break;
    case starter_port_PARITY_ODD:
        ei_encode_atom(resp, &resp_index, "odd");
        break;
    case starter_port_PARITY_SPACE:
        ei_encode_atom(resp, &resp_index, "space");
        break;
    case starter_port_PARITY_MARK:
        ei_encode_atom(resp, &resp_index, "mark");
        break;
    case starter_port_PARITY_IGNORE:
        ei_encode_atom(resp, &resp_index, "ignore");
        break;
    }
    ...
    case starter_port_FLOWCONTROL_HARDWARE:
        ei_encode_atom(resp, &resp_index, "hardware");
        break;
    case starter_port_FLOWCONTROL_SOFTWARE:
        ei_encode_atom(resp, &resp_index, "software");
        break;
    }
...
static void handle_write(const char *req, int *req_index)
{
    ...
    // starter_port_write always invokes a callback when it completes (error or no error).
    starter_port_write(starter_port, to_write, amount_to_write, timeout);
}
...
static void handle_read(const char *req, int *req_index)
{
    ...
    starter_port_read(starter_port, timeout);

    // handle_read_completed is called when read completes, times out, or errors
}

/* Called when starter_port_read completes or fails */
...
static void handle_notify_read(int error_reason, const uint8_t *data, size_t len)
{
    ...
    else
    {
        ...
        ei_encode_tuple_header(resp, &resp_index, 2);
        ei_encode_atom(resp, &resp_index, "error");
        ei_encode_atom(resp, &resp_index, starter_port_last_error());
    }
    erlcmd_send(resp, resp_index);
    free(resp);
}
...
static void handle_drain(const char *req, int *req_index)
{
    ...
    if (starter_port_drain(starter_port) >= 0)
        send_ok_response();
    else
        send_error_response(starter_port_last_error());
}
...
static void handle_flush(const char *req, int *req_index)
{
    ...
    enum starter_port_direction direction;
    if (strcmp(dirstr, "receive") == 0)
        direction = starter_port_DIRECTION_RECEIVE;
    else if (strcmp(dirstr, "transmit") == 0)
        direction = starter_port_DIRECTION_TRANSMIT;
    else if (strcmp(dirstr, "both") == 0)
        direction = starter_port_DIRECTION_BOTH;
    else
    {
        send_error_response("einval");
        return;
    ...
    if (starter_port_flush(starter_port, direction) >= 0)
        send_ok_response();
    else
        send_error_response(starter_port_last_error());
}
...
static void handle_set_rts(const char *req, int *req_index)
{
    ...
    if (starter_port_set_rts(starter_port, !!val) >= 0)
        send_ok_response();
    else
        send_error_response(starter_port_last_error());
}
...
static void handle_set_dtr(const char *req, int *req_index)
{
    ...
    if (starter_port_set_dtr(starter_port, !!val) >= 0)
        send_ok_response();
    else
        send_error_response(starter_port_last_error());
}
...
static void handle_set_break(const char *req, int *req_index)
{
    ...
    if (starter_port_set_break(starter_port, !!val) >= 0)
        send_ok_response();
    else
        send_error_response(starter_port_last_error());
}
...
static void handle_signals(const char *req, int *req_index)
{
    ...
    struct starter_port_signals sig;
    if (starter_port_get_signals(starter_port, &sig) >= 0)
    {
        char resp[128];
        int resp_index = sizeof(uint16_t);
        resp[resp_index++] = response_id;
...
void main_loop()
{
    starter_port_default_config(&current_config);
    if (starter_port_init(&starter_port,
                          handle_write_completed,
                          handle_read_completed,
                          handle_notify_read) < 0)
        errx(EXIT_FAILURE, "starter_port_init failed");
    ...
    for (;;)
    {
        ...
        int count = starter_port_add_poll_events(starter_port, &fdset[1], &timeout);
        ...
        // Call starter_port_process if it added any events
        if (count)
            starter_port_process(starter_port, &fdset[1]);
    }

    // Exit due to Erlang trying to end the process.
    //
    if (starter_port_is_open(starter_port))
        starter_port_flush_all(starter_port);
}
...
void main_loop()
{
    ...
    while (running)
    {
        ...
        DWORD count = 1 + starter_port_add_wfmo_handles(starter_port, &handles[1], &timeout);
        ...
        case WAIT_OBJECT_0 + 1:
            starter_port_process_handle(starter_port, handles[1]);
            break;

        case WAIT_OBJECT_0 + 2:
            starter_port_process_handle(starter_port, handles[2]);
            break;
...
int main(int argc, char *argv[])
{
    ...
    char logfile[64];
#ifdef __WIN32__
    sprintf(logfile, "starter_port-%d.log", (int)GetCurrentProcessId());
#else
    sprintf(logfile, "/tmp/starter_port-%d.log", (int)getpid());
#endif
