 ...
 * @return <0 on error
 */
static int starter_port_config_line(int fd, const struct starter_port_config *config)
{
 ...
 * @return <0 on error
 */
static int starter_port_config_flowcontrol(int fd, const struct starter_port_config *config)
{
    struct termios options;
...
{
    char *starter_port_path = name_to_device_file(name);
    if (!starter_port_path)
    {
        debug("Can't convert '%s' to starter_port path", name);
        return -1;
    }

    port->fd = open(starter_port_path, O_RDWR | O_NOCTTY | O_CLOEXEC | O_NONBLOCK);
    free(starter_port_path);
    ...
    if (starter_port_config_line(port->fd, config) < 0)
    {
        debug("starter_port_config_line failed");
        goto handle_error;
    }

    if (starter_port_config_flowcontrol(port->fd, config) < 0)
    {
        debug("starter_port_config_flowcontrol failed");
        goto handle_error;
    }
...
{
    debug("starter_port_close_on_error %d, %d", port->fd, reason);
...
void starter_port_write(struct starter_port *port, const uint8_t *data, size_t len, int timeout)
{
    ...
    do
    {
        ...
        debug("starter_port_write: wrote %d/%d, errno=%d (%s) data=%p, fd=%d",
              (int)written,
              (int)len,
    ...
    if (written < 0)
    {
        ...
        else
        {
            ...
            starter_port_close_on_error(port, reason);
        }
    }
    else
    {
...
static void continue_in_progress_write(struct starter_port *port)
{
...
static void process_read(struct starter_port *port)
{
    ssize_t bytes_read;
    do
    {
